    ...
    'success': function(data){
      ...
      for (var k = 0; k < data.response.length; k++){
        for (j = 0; j < giorni; j++){
          ...
          var giornofesta = moment(data.response[k].date).format('DD');
          if($('li').eq(j).attr('data-giorno') == giornofesta){
            $('ul li').eq(j).append(' ' + data.response[k].name);
          }
        }
      }
    },
